...
function result = simdata_check(data,para1,para2,para3,para4,cum_ILILAB,cutoff)

data1 = data;

age_prob = [sum(data(:,5) > 0 & data(:,5) <= 18) sum(data(:,5) > 18 & data(:,5) <= 50) sum(data(:,5) > 50)];
data1(data(:,5) == -1,5) = randsample([17 39 55],sum(data(:,5) == -1),1,age_prob);

data1(data(:,8)==-1,8) = 5*2.^((data(data(:,8)==-1,5)<=18).*randsample(0:9,sum(data(:,8)==-1),1,para2(1:10))' + (data(data(:,8)==-1,5)>18).*randsample(0:9,sum(data(:,8)==-1),1,para2(11:20))');
